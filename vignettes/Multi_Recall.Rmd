...
The participant data should be in long format with each answer as one row of data. The participant ID will be repeated by participant, while the trial ID will be repeated by trial.  

```{r}
DF_long <- arrange_data(data = multi_test,
      responses = "Response",
      sep = " ",
      id = "Sub.ID")
head(DF_long)
...
```{r}
multi_data$Response <- tolower(multi_data$Response)
