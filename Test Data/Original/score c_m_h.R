####set up####
dat = read.csv("Cates Maxwell Huff.csv")

dat = dat[ , c(2, 6, 16, 19)]

colnames(dat)[1] = "Sub.ID"
colnames(dat)[2] = "Cue"
colnames(dat)[4] = "manually_coded"

dat$Cue = sub("^[^-]*", "", dat$Cue)
dat$Cue = substring(dat$Cue, 3)
...
cmh_matched = percent_match(dat$Response, key = dat$Key, id = dat$Sub.ID)

####Score the data####
#going to use 10 cutoff points: 100, 90, 85, 80, 75, 70, 65, 60, 55, 50

#first add percent match
dat$percent_match = cmh_matched$percent_match

#100%
score_recall(cmh_matched, set.cutoff = 1)
...
#90%
score_recall(cmh_matched, set.cutoff = .90)
...
#85%
score_recall(cmh_matched, set.cutoff = .85)
...
#80%
score_recall(cmh_matched, set.cutoff = .80)
...
#75%
score_recall(cmh_matched, set.cutoff = .75)
...
#70%
score_recall(cmh_matched, set.cutoff = .70)
...
#65%
score_recall(cmh_matched, set.cutoff = .65)
...
#60%
score_recall(cmh_matched, set.cutoff = .60)
...
#55%
score_recall(cmh_matched, set.cutoff = .55)
...
#50%
score_recall(cmh_matched, set.cutoff = .50)
